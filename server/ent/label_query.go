...
func (lq *LabelQuery) QueryTodos() *TodoQuery {
	query := (&TodoClient{config: lq.config}).Query()
...
func (lq *LabelQuery) WithTodos(opts ...func(*TodoQuery)) *LabelQuery {
	query := (&TodoClient{config: lq.config}).Query()
...
func (lq *LabelQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Label, error) {
	...
	if query := lq.withTodos; query != nil {
		if err := lq.loadTodos(ctx, query, nodes,
			func(n *Label) { n.Edges.Todos = []*Todo{} },
			func(n *Label, e *Todo) { n.Edges.Todos = append(n.Edges.Todos, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *LabelQuery) loadTodos(ctx context.Context, query *TodoQuery, nodes []*Label, init func(*Label), assign func(*Label, *Todo)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(label.TodosTable)
		s.Join(joinT).On(s.C(todo.FieldID), joinT.C(label.TodosPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(label.TodosPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(label.TodosPrimaryKey[0]))
